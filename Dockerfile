...
ARG USER_NAME="eric-oss-2pp-egookat-poc-28"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG JAR_FILE
ADD target/${JAR_FILE} eric-oss-2pp-egookat-poc-28-app.jar
...
CMD ["/bin/sh", "-c", "java ${JAVA_OPTS} -Dcom.sun.management.jmxremote=true \
    ...
    -jar eric-oss-2pp-egookat-poc-28-app.jar"]
...
LABEL \
    org.opencontainers.image.title=eric-oss-2pp-egookat-poc-28-jsb \
